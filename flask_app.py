    ...
    if request.method == 'POST':
        ...
        if query:
            ...
            if len(active_ais) < 2:
                ...
                prompt = f"Simulate a global expansion strategy for: {query}. Provide target markets, entry strategy, and estimated costs (USD/year). Be concise."
                responses = {}
                for ai, config in active_ais.items():
                    try:
                        key = session[f'{ai}_key']
                ...
                vote_prompt = (
                    ...
                    f"Vote for the most viable and cost-effective strategy. Reply ONLY with the number (1-{len(ai_list)})."
                )
                ...
                best_answer = responses[best_ai.lower()]
                result = (
                    f"**Best Global Expansion ({votes[best_num]} votes):** {best_ai} wins!\n\n{best_answer}\n\n"
    ...
    if request.method == 'POST':
        ...
        if query:
            ...
            if len(active_ais) < 2:
                ...
                prompt = f"Conduct an AI ethics audit for: {query}. Identify ethical risks, compliance issues, and mitigation steps. Be concise."
                responses = {}
                for ai, config in active_ais.items():
                    try:
                        key = session[f'{ai}_key']
                ...
                vote_prompt = (
                    ...
                    f"Vote for the most thorough and actionable audit. Reply ONLY with the number (1-{len(ai_list)})."
                )
...
def rd_breakthrough():
    ...
    if request.method == 'POST':
        ...
        if query:
            active_ais = {ai: config for ai, config in AI_CONFIGS.items() if session.get(f'{ai}_key')}
            if len(active_ais) < 2:
                result = "Need 2+ API keys for R&D breakthrough."
            else:
                prompt = f"Generate an R&D breakthrough idea for: {query}. Provide concept, feasibility, and potential impact. Be concise."
                responses = {}
                for ai, config in active_ais.items():
                    try:
                        key = session[f'{ai}_key']
                ...
                vote_prompt = (
                    ...
                    f"Vote for the most innovative and feasible idea. Reply ONLY with the number (1-{len(ai_list)})."
                )
